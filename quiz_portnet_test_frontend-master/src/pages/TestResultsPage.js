...
function TestResultsPage() {
  ...
  useEffect(() => {
    if (selectedTest) {
      setLoading(true);
      axios.get(`http://localhost:8088/api/tests/${selectedTest}/candidates`)
      ...
      <div style={{ display: 'flex', width: '100%', marginTop: '64px', backgroundColor: '#D9D9D9' }}>
        ...
        <Container sx={{ width: '80%', padding: 4, overflowY: 'auto' }}>
          {loading ? (
            ...
            <Typography variant="h6" color="textSecondary">Sélectionnez un test pour voir les résultats.</Typography>
          )}
        </Container>
      </div>
    </div>
